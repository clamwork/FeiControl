...
> "像打开游戏面板一样，看你的 Agent 世界。"

---

## 📌 核心定位（一句话定义）

> **FeiControl 是一个让你玩着玩着就把 Agent 管好了的命令中心。**

它本质是 Agent 管理工具——帮你跟 Agent 对话、看状态、查记忆、配任务。
但它的**体验**不是工具——你打开它的感觉，像打开游戏面板：

- 有漂亮的 3D 办公室，Agent 在里面走来走去
- 点一下 Agent，卡片翻转、数据刷出来
- 聊多了，你和 Agent 之间有一条"默契度"在涨
- 任务跑完，有进度条跳满的瞬间
- 页面切换，有自然的过渡动画

**FeiControl 做的事（管理 Agent）和其他工具一样。不一样的是你做这些事的时候，感觉是控着、舒服的、还想再打开。**

### 什么是对的事情

| ✅ 做 | ❌ 不做 |
|-------|---------|
| 让"查看 Agent 状态"这个动作本身有反馈感、满足感 | 不牺牲体验去堆功能——少一个功能但每一次点击都舒服 |
| 用 Adapter 连不同的 Agent 框架，让你不管用什么 Agent 都能用 FeiControl | 不做内置 Agent 运行时——你把 Agent 交给谁，FeiControl 就帮你管谁 |
| 游戏面板般的 UI：进度条、卡片翻转、收集进度、成就徽章 | 不做"看起来像 Grafana"的东西——工具脸和游戏脸是两回事 |
| PWA 离线可用——关了电脑，面板还在本地存着 | 不做数据大盘——看数据不是目的，**看数据的过程中觉得爽**才是目的 |
| 好看、好玩、让你每次都忍不住点开看看 | 不做企业级严肃仪表盘——严肃是给老板看的，FeiControl 是给你这种"玩 Agent"的人玩的 |
| 你随时可以关掉，不依赖你 | 不做 Daily Active User 那种指标压迫——你想开就开，不想开就不开 |
...
> 让管理 Agent 这件事，变得像玩游戏一样让人想一直打开。

### 产品使命

FeiControl 做一件事：**让你每次操作 Agent 的时候，感觉是在玩，而不是在工作。**

不是陪你、不是等你——是你在玩它的过程中，不知不觉就把事情做了。

1. **点一下就有反馈** — 点 Agent → 卡片翻转。发消息 → 回复冒出来。任务完成 → 进度条跳满。每一个动作都有一个让你**舒服的瞬间**。
2. **有收集的乐趣** — 每个 Agent 是不同的角色，有不同的外观、名字、个性。你看到它们一个个出现在办公室里，想集齐。
3. **有进步的满足感** — 你和 Agent 聊得越多，默契度在涨。数据会告诉你"今天协助了你 X 次"。你在变强。
4. **全程可控** — 你在操作面板。面板不会自己动。没有自动执行、没有你不知道的事。你是玩家，不是观众。

### 为什么需要 FeiControl

| 场景 | 没有 FeiControl | 有 FeiControl |
|------|----------------|---------------|
| 你打开电脑想看 Agent 怎么样了 | 打开终端 → 输入命令 → 看一堆冷文本 | 打开浏览器 → Agent 在 3D 办公室坐着 → 头上顶着状态气泡 |
| 你跟 Agent 对话 | 终端敲指令，像在调 API | 打开聊天面板，消息有动画、头像会动、对话记录是时间线 |
| 看 Agent 做了什么 | 翻日志文件，一行一行找 | 卡片上直接显示"今天执行了 5 个任务" + 进度条 |
| 同时管理多个 Agent | 终端切来切去，记不住谁是谁 | 一个页面，每个 Agent 是独立的角色卡片 |
| 想知道自己和 Agent 的关系 | 没有指标 | "默契度 78%"，"本周合作 23 次" |
| 3D 办公室 | ❌ | 🌸 每个 Agent 有工位，你在办公室里走来走去 |

### 产品哲学（7 条）

1. **反馈优先于功能** — 每个操作必须有即时、愉悦的反馈。没有动画的点击不如不做。宁愿少一个功能，也要每次点击都舒服。
2. **可控感优先于自动化** — 你是玩家，不是观众。所有动作由你触发，没有静默操作、没有你不知道的自动行为。
3. **收集的乐趣** — 每个 Agent 是不同的角色，有外观、性格、默契度。你看到它们就想"再多一个"。
4. **进度感** — 进度条、数字、百分比。让人想"再练一点"、"再涨一点"。像游戏里升级的愉悦感。
5. **审美是玩的一部分** — 游戏画面不漂亮就没人想玩。FeiControl 也是。好看不是装饰，是核心体验。
6. **Local First** — 离线可用。面板在本地，数据在本地。不依赖网络，像单机游戏一样随时打开。
7. **Agent Neutral** — 不管你的 Agent 是 OpenClaw、Hermes 还是别的，FeiControl 都是一个面板。它不关心后面跑的是什么。
...
FeiControl 的用户只有一个共同点：**你希望管理 Agent 的过程本身是有趣的，而不是又多了一件要做的事。**

| 画像 | 描述 |
|------|------|
| **身份** | AI 爱好者、独立开发者、创客——在用 Agent 的人 |
| **技术背景** | 会用终端、有自己的 Agent 实例（OpenClaw / Hermes / 其他） |
| **使用习惯** | Agent 不是实验，是日常——你每天都和 Agent 交互 |
| **你现在的感觉** | 终端操作太冷，Dashboard 太严肃。你想要**操作起来舒服的东西** |
| **你需要的** | 不是"管理工具"。是一个你**愿意打开、操作起来满足、关掉也不累**的面板 |
...
```
1. 下载安装，设好密码
2. 打开浏览器 → FeiControl 自动检测到你的 Agent
3. 3D 办公室里亮起一个工位，Agent 坐在那里
4. 你点了一下 Agent 的卡片 → 卡片翻转，数据刷出来
5. 进度条跳了一下。你忍不住又点了一下。
```

不是"安装完成"。是**你第一次觉得：管理 Agent 原来可以这么舒服。**

#### 🔴 日常

```
早晨：
  打开 FeiControl，看昨晚 Agent 跑了什么
  → 卡片上："昨晚执行 5 个任务，成功率 100%"
  → 进度条跳满，默契度 +1。你点了确认。

工作中：
  想跟 Agent 说点什么 → 打开聊天面板
  → 消息冒出来有动画，头像会微微动
  → 你发了一条，它回了。对话记录排成时间线。
  → 你看了一眼默契度：72%。"再聊一会儿吧。"
...
下午：
  又装了一个新 Agent
  → FeiControl 自动识别，办公室里多了一个工位
  → 两个 Agent 坐在一起。你点开新的那张卡片。
  → "今天的收集进度：2/8"
```

FeiControl 不帮你做决定。它只是让**每次操作都顺手、有反馈、让人想继续。**

#### 🟢 偶尔满足

```
忙了一周没打开：
  打开 FeiControl → Agent 攒了一周的数据
  → "7 天协助你处理了 32 件事"
  → 数字在涨。默契度从 70% 到了 74%。
  → 你划过一张张卡片，像是打开游戏后看到离线收益。

深夜睡不着：
  打开 FeiControl，单纯想看看
  → 3D 办公室里，Agent 在工位上趴着
  → 你转了转视角，看了会儿它
  → 关掉。不是因为有事做才打开。是因为打开它本身挺舒服的。
```

不是"等你回来"。是**你每次操作它的时候，都感觉有点满足。**

FeiControl 不会告诉你应该做什么。你想点就点，不想点就关。但你知道，每次打开，至少有一件事是确定的——**操作它的这几下，是舒服的。**
